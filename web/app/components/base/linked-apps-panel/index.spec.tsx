...
vi.mock('@/app/components/base/app-icon', () => ({
  default: vi.fn(({ iconType, icon, background, imageUrl, size }: { iconType: 'image' | 'emoji' | 'link' | null, icon: string, background: string, imageUrl: string, size: string }) => (
    <div
...
describe('LinkedAppsPanel', () => {
  ...
  it('passes correct props to AppIcon', () => {
    render(<LinkedAppsPanel relatedApps={mockRelatedApps} isMobile={false} />)

    const icons = screen.getAllByTestId('app-icon')

    // First icon checks
    expect(icons[0]).toHaveAttribute('data-icon-type', 'emoji')
    expect(icons[0]).toHaveAttribute('data-icon', '🤖')
    expect(icons[0]).toHaveAttribute('data-background', '#FFEAD5')
    expect(icons[0]).toHaveAttribute('data-size', 'tiny')

    // Second icon checks
    expect(icons[1]).toHaveAttribute('data-icon-type', 'image')
    expect(icons[1]).toHaveAttribute('data-image-url', 'https://example.com/icon.png')
  })
