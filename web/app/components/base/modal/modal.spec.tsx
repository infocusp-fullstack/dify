...
vi.mock('@/app/components/base/portal-to-follow-elem', () => ({
  ...
  PortalToFollowElemContent: ({ children, className, onClick }: {
    children: React.ReactNode
    className?: string
    onClick?: () => void
  }) => (
    <div data-testid="portal-content" className={className} onClick={onClick}>
      {children}
    </div>
  ),
}))
...
describe('Modal Component', () => {
  ...
  it('calls onClose when close icon is clicked', () => {
    ...
    const closeIcon = document.querySelector('.remixicon')?.parentElement
  ...
  it('calls onConfirm when confirm button is clicked', () => {
    ...
    fireEvent.click(screen.getByText('Confirm Me'))
  ...
  it('applies correct size classes', () => {
    ...
    let container = screen.getByText('Test Modal').parentElement
    expect(container).toHaveClass('w-[480px]')

    rerender(<Modal {...defaultProps} size="md" />)
    container = screen.getByText('Test Modal').parentElement
    expect(container).toHaveClass('w-[640px]')
  })
  ...
  it('disables buttons when disabled prop is true', () => {
    ...
    expect(screen.getByText('common.operation.cancel').closest('button')).toBeDisabled()
